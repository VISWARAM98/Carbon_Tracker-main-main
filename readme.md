...
**Last Updated**: April 1, 2026 (Complete Functions & Buttons Reference Added)
**Version**: 2.0 (Complete Technical Documentation)
**Python Version**: 3.8+
**Flask Version**: 2.0+
**Status**: ✅ All critical bugs fixed, ESG modules fully functional, Complete documentation

---

## 📚 **COMPLETE FUNCTIONS & BUTTONS REFERENCE** (APRIL 2026)

### **Quick Links to Documentation**
- [All 70+ Routes & Functions](#all-routes--functions)
- [Buttons on Every Page](#buttons--ui-elements)
- [How Reports are Generated](#report-generation-process)
- [Calculation Functions](#calculation-functions)
- [AI & Analytics Functions](#ai--analytics-functions)
...
| **Dashboard Routes** | 6 | /dashboard, /report, /sentinel, /services |
| **ESG Routes** | 9 | /esg-data-center, /esg-environmental, /esg-social, /esg-governance, /esg-supply-chain |
| **Compliance Routes** | 8 | /compliance_dashboard, /api/compliance/matrix, /api/compliance/update-status |
| **Environmental Routes** | 8 | /api/environmental/create, /api/environmental/list, /api/environmental/update/<id> |
| **Social Routes** | 6 | /api/social/save, /api/social/data, /api/social/survey/create |
| **Governance Routes** | 2 | /api/governance/save, /api/governance/data |
| **Supply Chain Routes** | 6 | /api/supply_chain/suppliers, /api/supply_chain/invite, /api/supply_chain/resend |
| **AI & Document Routes** | 4 | /ai-consultant, /api/gemini-advice, /api/analyze-document |
| **Carbon Tools** | 4 | /carboniq, /carbon_roadmap, /api/roadmap/emissions, /api/roadmap/generate |
| **Marketplace Routes** | 3 | /marketplace, /api/marketplace/enquiry, /api/marketplace/listing |
| **Evidence & Audit** | 2 | /api/evidence/upload, /api/evidence/link |
| **Other Routes** | 11 | /profile_settings, /company_settings, Survey routes, Dashboard stats |
| **TOTAL** | **73+** | **60+ unique functions** |

### **Buttons & Their Functions**

#### **Navigation Bar Buttons**
- **Logo/Home** 🏠 → `/dashboard`
- **Audit Dropdown** 📊 → ESG Data Center, Compliance, Source Docs, Verification, Audit Trail
- **Services Dropdown** 🛠️ → AI Consultant, Reports, Marketplace, CarbonIQ, Roadmap, Surveys
- **User Profile Menu** 👤 → Settings, Company Settings, Logout

#### **Dashboard Page Buttons**
- **Add Emission Data** ➕ → Opens modal to manually add emissions
- **Analyze Document** 📸 → Upload files for OCR analysis
- **My Report** 📄 → Navigate to GRI/GHG report page
- **Download Report** 📥 → Export as PDF via `/api/generate-report-pdf`

#### **ESG Data Center Buttons**
- **Environmental Card** 🌱 → `/esg-environmental` - Energy, Water, Emissions, Waste tracking
- **Social Card** 👥 → `/esg-social` - Workforce, Safety, Training, Community data
- **Governance Card** ⚖️ → `/esg-governance` - Risk, Compliance, Ethics, Board data
- **Supply Chain Card** 🏭 → `/esg-supply-chain` - Supplier data, Scope 3 emissions

#### **Module-Specific Buttons**
- **Save Button** 💾 → POST data to respective `/api/` endpoint
- **Submit for Review** ✅ → Change status to submitted
- **Attach Evidence** 📎 → Upload supporting documents
- **Create Survey** 📋 → Generate employee survey with token
- **Import Responses** 📊 → Integrate survey results into social data

#### **Sentinel (AI Consultant) Buttons**
- **Send Message** ✈️ → POST to `/api/gemini-advice` with Gemini AI
- **Quick Questions** 💡 → Pre-defined questions trigger AI analysis
- **How to Fix** 🔧 → Get reduction strategies from AI
- **New Chat** ➕ → Clear chat and restart conversation

#### **Compliance Dashboard Buttons**
- **Update Status** → Mark as compliant/partial/non-compliant
- **Upload Evidence** 📄 → Attach proof document
- **Generate Report** 📋 → Create compliance assessment report
- **View Mapping** 🔗 → See relationships between standards

### **Report Generation Process**

#### **GRI/GHG Report** (When user clicks "Download Report")
1. **Data Fetching** → Query all emissions by scope (1, 2, 3)
2. **Calculations** → Calculate totals, intensity ratios, trends
3. **Structure** → Build report with:
   - Report metadata
   - GRI 102 (organizational info)
   - GHG Protocol alignment
   - Emissions inventory
   - Performance metrics
   - Risk assessment
   - Recommendations
4. **PDF Generation** → Use ReportLab to create file
5. **Download** → Stream to browser as PDF file

#### **Compliance Report** (POST to `/api/compliance/generate-report`)
1. **Matrix Generation** → 9 standards × ~45 clauses
2. **Status Check** → Get compliance status for each clause
3. **Evidence Linking** → Attach uploaded documents
4. **Risk Assessment** → Flag non-compliant items
5. **PDF Export** → Generate assessment report

#### **Roadmap Report** (POST to `/api/roadmap/generate`)
1. **Historical Analysis** → Analyze emission trends
2. **Target Setting** → Propose SBTi-aligned targets
3. **Action Planning** → Suggest reduction initiatives
4. **Timeline** → Create phased implementation schedule
5. **Investment Calc** → Estimate costs & ROI

### **Key Calculation Functions**

- **`calculate_emissions(scope, category, amount, unit)`** → CO₂e calculation
- **`calculate_energy_intensity(energy_data)`** → CO₂e per kWh
- **`calculate_water_intensity(water_data)`** → CO₂e per m³
- **`calculate_waste_intensity(waste_data)`** → CO₂e per ton
- **`calculate_environmental_intensities(username)`** → All intensity metrics

### **AI Functions**

- **`get_gemini_advice(type, message)`** → Google Gemini AI response
- **`generate_ai_advice(user_data, type)`** → Fallback advice system
- **`analyze_document(file)`** → OCR + AI text extraction
- **`generate_reduction_advice(analysis)`** → Reduction strategies
- **`generate_compliance_advice(analysis)`** → Regulatory guidance

### **Full Documentation File**

For complete detailed documentation of all 73+ routes, 40+ functions, all buttons with parameters, report generation workflows, calculation methods, database models, and JavaScript functions, see: **`COMPLETE_FUNCTIONS_REFERENCE.md`**
